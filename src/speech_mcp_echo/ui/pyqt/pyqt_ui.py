...
from speech_mcp_echo.state_manager import StateManager
...
from speech_mcp_echo.ui.components import (
    AudioVisualizer, 
    AnimatedButton, 
    TTSAdapter, 
    AudioProcessorUI
...
from speech_mcp_echo.config import get_env_setting, get_setting, set_setting, set_env_setting
...
class PyQtSpeechUI(QMainWindow):
    ...
    def __init__(self):
        ...
        self.summarizer_enabled = True  # Default to enabled
    ...
    def setup_ui(self):
        ...
        self.engine_combo.setStyleSheet("""
            ...
            padding: 5px;
        """)
        self.engine_combo.addItem("Google Cloud TTS", "google")
        self.engine_combo.addItem("OpenAI TTS", "openai")
        ...
        self.stt_label = QLabel("STT: Loading...")
        self.stt_label.setStyleSheet("""
            font-size: 11px;
            color: #00c8ff;
            ...
            padding: 4px 8px;
        """)
        status_layout.addWidget(self.stt_label)

        # TTS Engine indicator
        self.tts_label = QLabel("TTS: Loading...")
        self.tts_label.setStyleSheet("""
            ...
            padding: 4px 8px;
        """)
        status_layout.addWidget(self.tts_label)

        # Summarizer toggle
        self.summarizer_label = QLabel("Summarizer: Loading...")
        self.summarizer_label.setStyleSheet("""
            font-size: 11px;
            color: #ffa500;
            background-color: #1a1a1a;
            border-radius: 3px;
            ...
            cursor: pointer;
        """)
        self.summarizer_label.setToolTip("Click to toggle summarization on/off")
        self.summarizer_label.mousePressEvent = self.toggle_summarizer
        status_layout.addWidget(self.summarizer_label)
    ...
    def update_voice_list(self):
        ...
        for i, voice in enumerate(voices):
            display_name = self._format_voice_name(voice)
    ...
    def sync_engine_selector(self):
        """Sync the engine selector dropdown to the current TTS engine"""
        if not hasattr(self, 'tts_adapter') or not self.tts_adapter:
            return

        current_engine = self.tts_adapter.get_current_engine()
        if not current_engine:
            return

        # Block signals to avoid triggering on_engine_changed
    ...
    def _format_voice_name(self, voice: str) -> str:
        """Format voice ID for display in dropdown."""
        # OpenAI voices - simple names with descriptions
        openai_voices = {
            "alloy": "Alloy (OpenAI - Neutral)",
            "echo": "Echo (OpenAI - Male)",
            "fable": "Fable (OpenAI - British)",
        ...
        # Google TTS voices - parse language-region-type-letter format
        # e.g., "cmn-TW-Wavenet-A" -> "Chinese TW Wavenet A"
        if "-" in voice:
            parts = voice.split("-")
            if len(parts) >= 3:
                lang_map = {
                ...
                lang = lang_map.get(parts[0], parts[0].upper())
                region = parts[1]
                quality = parts[2] if len(parts) > 2 else ""
                letter = parts[3] if len(parts) > 3 else ""
                return f"{lang} {region} {quality} {letter} (Google)"

        # Fallback - use voice ID as-is
        return voice
    ...
    def initialize_tts_adapter(self):
        ...
        try:
            ...
            # Update engine selector and voice list when TTS is ready - use QTimer to call from main thread
            QTimer.singleShot(0, self.sync_engine_selector)
    ...
    def update_engine_indicators(self):
        """Update the engine status indicators"""
        try:
            # Get STT engine from config
            stt_engine = get_setting("stt", "engine", "faster-whisper")
            self.stt_label.setText(f"STT: {stt_engine}")

            # Get TTS engine from config
            tts_engine = get_setting("tts", "engine", "google")
            self.tts_label.setText(f"TTS: {tts_engine}")

            # Get summarizer status from config
            summarizer_enabled = get_setting("summarizer", "enabled", True)
            self.summarizer_enabled = summarizer_enabled
            self.update_summarizer_label()
        except Exception as e:
            self.stt_label.setText("STT: Unknown")
            self.tts_label.setText("TTS: Unknown")
            self.summarizer_label.setText("Summarizer: Unknown")

    def toggle_summarizer(self, event):
        """Toggle summarization on/off"""
        try:
            # Toggle the state
            self.summarizer_enabled = not self.summarizer_enabled

            # Save to config
            set_setting("summarizer", "enabled", self.summarizer_enabled)

            # Update the label
            self.update_summarizer_label()

            # Provide feedback
            status = "enabled" if self.summarizer_enabled else "disabled"
            self.transcription_label.setText(f"Summarizer {status}")
            QTimer.singleShot(2000, lambda: self.transcription_label.setText("Ready for voice interaction"))
        except Exception as e:
            self.transcription_label.setText(f"Error toggling summarizer: {str(e)}")

    def update_summarizer_label(self):
        """Update the summarizer label based on current state"""
        if self.summarizer_enabled:
            self.summarizer_label.setText("Summarizer: ON")
            self.summarizer_label.setStyleSheet("""
                font-size: 11px;
                color: #00ff64;
                background-color: #1a1a1a;
            ...
            self.summarizer_label.setText("Summarizer: OFF")
            self.summarizer_label.setStyleSheet("""
                font-size: 11px;
                color: #ff4444;
                background-color: #1a1a1a;
    ...
    def on_engine_changed(self, index):
        ...
        # Switch engine
        self.transcription_label.setText(f"Switching to {engine.upper()} TTS...")
        if self.tts_adapter.set_engine(engine):
            self.update_voice_list()
            self.tts_label.setText(f"TTS: {engine.capitalize()}")
            self.transcription_label.setText(f"Switched to {engine.upper()} TTS")
        else:
            self.transcription_label.setText(f"Failed to switch to {engine.upper()} TTS")
