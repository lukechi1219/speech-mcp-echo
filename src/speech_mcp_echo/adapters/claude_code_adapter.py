"""
Claude Code adapter for speech-mcp-echo.

Integrates voice capabilities with Claude Code CLI.

Integration methods:
1. MCP Server - Claude Code can connect to this as an MCP server
2. Hooks - Voice commands triggered by Claude Code hooks
3. Slash commands - Custom /voice commands

For now, this adapter extends the MCP adapter since Claude Code
...
class ClaudeCodeAdapter(ProtocolAdapter):
    ...
    Provides voice integration for Claude Code CLI via MCP protocol.
    Claude Code natively supports MCP servers, so this adapter provides
    MCP tools specifically designed for Claude Code workflows.

    Tools provided:
    - voice_listen: Listen for voice input and return transcription
    - voice_speak: Speak text using TTS (with optional summarization)
    - voice_conversation: Start a voice conversation loop
    ...
    def run(self, engine: "VoiceEngine") -> None:
        """
        Run the Claude Code adapter.

        Starts an MCP server with Claude Code-specific tools.
        ...
        def voice_config(
            ...
            summarizer_enabled: bool = None,
            summarizer_personality: str = None,
        ) -> str:
            """
        ...
        def voice_status() -> str:
            ...
            try:
                ...
                status_lines.append(f"STT Engine: {engine.config['stt']['engine']}")
            ...
            try:
                ...
                status_lines.append(f"TTS Engine: {engine.config['tts']['engine']}")
        ...
        # Run the MCP server
        logger.info("Starting Claude Code adapter (MCP server)...")
        mcp.run()
    ...
    def get_capabilities(self) -> dict:
        ...
        return {
            ...
            "tools": [
                ...
                {
                    "name": "voice_reply",
                    "description": "Speak and wait for response",
                },
                {
                    "name": "voice_config",
